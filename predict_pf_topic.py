...
def predict(gpt_model, gpt_tokenizer, conditioning_model, input_text, precondition_topk, postcondition_topk, length_cutoff, condition_lambda=1.0, device='cuda'):
    with torch.no_grad():
        ...
        while lengths.max() < length_cutoff:
            ...
            if i == 0:
                prev_condition_logits = torch.zeros_like(condition_logits)
                i += 1
            condition_logits = condition_logits - prev_condition_logits
            prev_condition_logits = torch.clone(condition_logits)
            
            # self-normalize
            #print('condition_logits1:', condition_logits)
            condition_logits = torch.log(F.softmax(condition_logits, dim=1) / (1 - F.softmax(condition_logits, dim=1)))#condition_logits - torch.log(sum(torch.exp(condition_logits)))#condition_logits / sum(condition_logits)
            ...
            selected = torch.multinomial(post_probs, 1).flatten()
            index_into_top_indices = post_indices[torch.arange(batch_size).to(post_indices.device), selected] # batch
            ...
            #resample
            encoded_input = encoded_input[torch.multinomial(post_probs[torch.arange(index_into_top_indices.shape[0]), selected], batch_size, replacement=True)]
        encoded_input = encoded_input[torch.multinomial(post_probs[torch.arange(index_into_top_indices.shape[0]), selected], 1)] 
