...
from predict_pf_topic import predict
...
def main(args):
    ...
    conditioning_model = Model(model_args, gpt_pad_id, 746265) #TODO don't hardcode this omg
    ...
    all_cr = []
    pair_num = 0
    for input_text in tqdm(prefixes, total=len(input_texts)):
        predict_function = predict
        condition_results = []
        for i in range(0, args.sample_size, args.max_sample_batch):
            num_samples = 50#min(args.max_sample_batch, args.sample_size - i)
            condition_results += predict_function(gpt_model, 
                            ...
                            device=args.device)
        all_cr.append((input_text, condition_results))
        pair_num += 1
        if args.max_pairs > 0 and pair_num >= args.max_pairs:
            break
    with open(args.log_file, 'w') as wf:
        writer = csv.DictWriter(wf, fieldnames=['input_text', 'generation'])
        writer.writeheader()
        for cr_group in all_cr:
            for cr in cr_group[1]:
                writer.writerow({'input_text': cr_group[0], 'generation': cr})
...
if __name__=='__main__':
    ...
    assert (args.condition_file is not None) != (args.prefix_file is not None) # one of two interfaces for specifying
