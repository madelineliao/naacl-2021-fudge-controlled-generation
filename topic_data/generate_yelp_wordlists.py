...
def main():
    ...
    k = 0
    top_k_train = []
    with open(f'../yelp_sorted_vocab_train.txt', 'r') as f:
        lines = f.readlines()
        for line in lines:
            line = line.strip()
            ...
            if k == args.k:
                break
    with open(f'yelp_train_wordlist.txt', 'w') as f:
        for word in top_k_train:
            f.write(word + '\n')
    top_k_test = []
    k = 0
    with open(f'../yelp_sorted_vocab_test.txt', 'r') as f:
        lines = f.readlines()
        for line in lines:
            line = line.strip()
            ...
            if k == args.k:
                break
    with open(f'yelp_test_wordlist.txt', 'w') as f:
